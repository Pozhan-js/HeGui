<template>
  <div class="app-container">
    ...
    <pro-table
      ...
      title=""
      api="/orderSalesDataStatistics/order/people/analysis"
      :columns="{
        orgName: '组织',
        shopName: '经销商名称',
        shopPhone: '经销商手机号',
        totalOrderNumber: '总下单人数',
        promoteOrderNumber: '推广下单人数',
        initiativeOrderNumber: '自然下单人数',
        scanCodeOrderNumber: '扫中奖码下单人数'
      }"
    />
...
import {
  orderSalesDataStatisticsOrderPeopleAnalysis,
  orderSalesDataStatisticsOrderPeopleAnalysisChannelType
} from '@/api/data/order'
...
export default {
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.drawCharts()
      setTimeout(this.$refs.orderTable.refresh)
    },
    drawCharts() {
      const params = {
        ...this.queryParams,
      ...
      orderSalesDataStatisticsOrderPeopleAnalysis(params).then(res => {
        const data = res.data
      ...
      orderSalesDataStatisticsOrderPeopleAnalysisChannelType(params).then(res => {
        const data = res.data
        const max = [
