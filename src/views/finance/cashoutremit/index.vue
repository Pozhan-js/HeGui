  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-col :span="1.5">
          <el-button
            v-hasPermi="['finance:cashout:supplier:remitexport']"
            size="mini"
            @click="handleExport"
          >导出
          </el-button>
    ...
    >
      ...
      />
      <el-table-column min-width="100" label="日志" align="center" prop="logging" />
      <el-table-column
        min-width="80"
        label="操作"
        align="center"
      ...
      >
        <template slot-scope="{ row }">
          <el-button
            ...
            v-hasPermi="['finance:cashout:supplier:remitagree']"
            size="mini"
            type="text"
            @click="handleRemit(row)"
          >打款
          </el-button>
          <el-button
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listCashout(this.getQuery()).then((response) => {
    ...
    handleRemit(row) {
      ...
      const message = isArray ? `是否确认批量打款?` : `是否确认给${row.shopName}进行打款?`
      remit({
        message: message,
        onInputComplete: arg => auditSupplierRemit({
          cashOutIds: ids,
          ...arg
        }),
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('cashout/supplier/remit/export', this.getQuery())
    },
    // 全选
