<template>
  ...
  <div v-else class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          v-hasPermi="['finance:distributionFeeRule:add']"
          type="primary"
          size="mini"
          @click="handleAdd"
        >新增配送费
      ...
      >
        ...
        <!-- <el-col :span="1.5">
          <el-button
            size="mini"
            @click="handleExport"
            v-hasPermi="['finance:distributionFeeRule:export']"
            >导出
          </el-button>
        </el-col> -->
      </el-row>
    ...
    <el-table
      ...
      :row-key="(row) => row.distributionFeeId"
      :data="distributionFeeRuleList"
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['finance:distributionFeeRule:expired']"
            size="mini"
            type="text"
          ...
          <el-button
            ...
            v-hasPermi="['finance:distributionFeeRule:enabled']"
            size="mini"
            type="text"
          ...
          <el-button
            v-if="scope.row.status != 3"
            v-hasPermi="['finance:distributionFeeRule:edit']"
          ...
          <el-button
            v-if="scope.row.status == 3"
            v-hasPermi="['finance:distributionFeeRule:remove']"
            size="mini"
            type="text"
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="来源" prop="accessMethod">
          <el-radio-group v-model="form.accessMethod">
            <el-radio v-for="(item,index) in accessMethodOptions" :key="index" :label="item.dictValue">{{ item.dictLabel }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
...
<script>
import {
  delDistributionFeeRule,
  listDistributionFeeRule,
  enabled,
  disabled,
  expired,
  recalculate
...
export default {
  ...
  created() {
    ...
    } else if (localStorage.getItem('DistributionFeeRule')) {
      this.queryParams = {
        ...JSON.parse(localStorage.getItem('DistributionFeeRule'))
      }
    }
    this.getList()
    // 查询组织
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listDistributionFeeRule(this.queryParams).then((response) => {
        this.searchParams = {
          ...this.queryParams
        }
        this.distributionFeeRuleList = response.rows
    ...
    resetQuery() {
      ...
      localStorage.removeItem('DistributionFeeRule')
      this.resetForm('queryForm')
      this.handleQuery()
    },
    /** 新增按钮操作 */
    handleAdd() {
      const path = '/finance/distributionFeeRule/add'
    ...
    /** 修改按钮操作 */
    handleUpdate(row) {
      const path = '/finance/distributionFeeRule/update'
      const resolver = this.$router.resolve(path)
      if (resolver.resolved?.path !== path) return this.warning('没有界面权限不支持跳转~')
      if (this.searchParams) {
        localStorage.setItem(
      ...
      this.$router.push({
        ...
        query: {
          type: 'update',
          distributionFeeId: row.distributionFeeId
        }
      })
    },
    handleCheck(row) {
    ...
    handleStand(rows, status) {
      this.$confirm(
        ...
        .then(() => {
          switch (status) {
            case 1:
              return enabled(rows.distributionFeeId)
            case 2:
              return disabled(rows.distributionFeeId)
            case 3:
              return expired(rows.distributionFeeId)
          }
        })
        .then(() => {
          this.getList()
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const distributionFeeIds = row.distributionFeeId || this.ids
      this.$confirm(
        `是否确认${
          typeof distributionFeeIds === 'object'
            ? '批量删除'
            : '删除"' + row.ruleName + '"'
        }配送费规则?`,
        '警告',
        ...
        .then(function() {
          return delDistributionFeeRule(distributionFeeIds)
        })
        .then(() => {
          this.getList()
    ...
    handleExport() {
      this.download(
        'finance/distributionFeeRule/export',
        {
          ...this.queryParams
        },
        `finance_distributionFeeRule.xlsx`
      )
    }
  }
}
