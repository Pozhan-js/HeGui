<template>
  <div class="app-container">
    ...
    <el-table ref="multipleTable" v-loading="loading" style="margin-top: 20px" :data="jobList" :row-key="(row) => row.jobId" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.checked = this.jobList.length === selection.length
    },
    // 任务状态修改
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$confirm(`是否确认${this.form.jobId != undefined ? '修改' : '新增'}定时任务?`, '警告', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            return this.form.jobId != undefined ? updateJob(this.form) : addJob(this.form)
          }).then(res => {
            this.msgSuccess(`${this.form.jobId != undefined ? '修改' : '新增'}成功~`)
            this.open = false
            this.getList()
          })
        }
