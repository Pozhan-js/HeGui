<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="配送员" prop="idTag">
        <el-input
          ...
          placeholder="请输入配送员姓名或手机"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="送货员" prop="deliveryTag">
        <el-input
          v-model="queryParams.deliveryTag"
          placeholder="请输入送货员姓名或手机"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
      ...
      >
        ...
        <el-col :span="1.5">
          <el-button
            v-hasPermi="['distribution:sell:infoexport']"
            size="mini"
            @click="handleExport"
          >导出
          </el-button>
    ...
    <el-table v-loading="loading" :max-height="maxHeight" :data="sellList">
      <el-table-column
        min-width="90"
        label="时间"
        align="center"
      ...
      <el-table-column min-width="100" label="配送员" align="center">
        <template slot-scope="scope">
          <div>{{ scope.row.distributionName }}</div>
          <div>{{ scope.row.distributionPhone }}</div>
        </template>
      </el-table-column>
      <el-table-column min-width="100" label="商品" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column
        min-width="100"
        :label="timeType === 'date' ? '当日采购量' : '当月采购量'"
        align="center"
      >
        <template slot-scope="scope">
          <div v-for="item in scope.row.list" :key="item.statisticsId">
...
<script>
import { listSell } from '@/api/order/courier'
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/distribution/sell/infoexport', this.queryParamFomat())
    }
  }
}
</script>
