<template>
  <div class="app-container">
    ...
    <el-tabs
      v-model="queryParams.hurryOrderStatus"
      class="tabContainer"
    ...
    >
      ...
      <el-tab-pane
        v-for="dict in orderHurryTabOptions"
        :key="dict.dictValue"
        :label="dict.dictLabel"
        :name="dict.dictValue"
      />
    ...
    <div v-loading="loading">
      <div class="order_title">
        ...
        <div style="width: 9.75%">用户信息</div>
        <div style="width: 9.75%">应收总额（元）</div>
        <div style="width: 9.75%">实付金额（元）</div>
        <div style="width: 9.75%">配送员</div>
        <div style="width: 9.75%">收货信息</div>
        <div style="width: 9.75%">订单状态</div>
        <div style="width: 9.75%">来源</div>
        <div style="width: 9.75%">历时（分）</div>
      </div>
      <div>
        <div
        ...
        >
          ...
          <div class="table">
            ...
            <div class="totalPrice">
              ...
              <div>{{ row.orderCustomer.poiAddress || '' }}</div>
            </div>
            <div class="totalPrice">
            ...
            <div class="totalPrice">{{ dateFormat(row.payToNowMinute) }}</div>
          </div>
...
<script>
import { getDistribution, listOrderInfo } from '@/api/order/hurry'
import { pushDistribution } from '@/api/order/transfer'
...
export default {
  ...
  created() {
    ...
    this.getDicts('order_hurry_tab').then((response) => {
      this.orderHurryTabOptions = response.data
    })
    this.getDicts('order_is_hurry').then((response) => {
      this.isHurryOptions = response.data
    })
  ...
  methods: {
    ...
    getList() {
      ...
      if (params.hurryOrderStatus === 'all') delete params.hurryOrderStatus
      if (this.queryParams.payTime && this.queryParams.payTime.length === 2) {
        params.orderBeginTime = this.queryParams.payTime[0]
        params.orderEndTime = this.queryParams.payTime[1]
      }
    ...
    // 是否催单
    isHurryFormat(status) {
      return this.selectDictLabel(this.isHurryOptions, status)
    },
    ...
    dateFormat(value) {
      ...
      return `${day > 0 ? day + '天' : ''}${hour > 0 ? hour : 0}时${minute}分`
    },
    /** 搜索按钮操作 */
    handleQuery() {
      this.queryParams.pageNum = 1
