...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.checked = this.typeList.length === selection.length
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
    ...
    submitForm: function() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.$confirm(
            `是否确定${
              this.form.dictId != undefined ? '修改' : '新增'
            }字典类型?`,
            '警告',
            {
            ...
            .then(() => {
              return this.form.dictId != undefined
                ? updateType(this.form)
                : addType(this.form)
            })
            .then((res) => {
              this.msgSuccess(
                `${this.form.dictId != undefined ? '修改' : '新增'}成功~`
              )
              this.open = false
              this.getList()
            })
