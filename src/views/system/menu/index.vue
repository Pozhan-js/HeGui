    ...
    >
      ...
      <el-table-column prop="icon" label="图标" align="center" width="100">
        <template slot-scope="scope">
          <svg-icon v-if="scope.row.icon" :iconClass="scope.row.icon"/>
        </template>
      </el-table-column>
      <el-table-column
        prop="orderNum"
  ...
  methods: {
    ...
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.$confirm(
            `是否确定${this.form.menuId != undefined ? "修改" : "新增"}菜单?`,
            "警告",
            {
            ...
            .then(() => {
              return this.form.menuId != undefined
                ? updateMenu(this.form)
                : addMenu(this.form);
            })
            .then((res) => {
              this.msgSuccess(
                `${this.form.menuId != undefined ? "修改" : "新增"}成功`
              );
              this.open = false;
              this.getList();
            });
