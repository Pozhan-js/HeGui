...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.checked = this.noticeList.length === selection.length
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset()
    ...
    submitForm: function() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.$confirm(
            `是否确定${this.form.noticeId != undefined ? '修改' : '新增'}公告?`,
            '警告',
            {
            ...
            .then(() => {
              return this.form.noticeId != undefined
                ? updateNotice(this.form)
                : addNotice(this.form)
            })
            .then((res) => {
              this.msgSuccess(
                `${this.form.noticeId != undefined ? '修改' : '新增'}成功~`
              )
              this.open = false
              this.getList()
            })
