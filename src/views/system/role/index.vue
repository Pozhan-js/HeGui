    ...
    >
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item v-show="form.dataScope == 2" label="数据权限">
          <el-checkbox
            v-model="orgExpand"
            @change="handleCheckedTreeExpand($event, 'org')"
          >展开/折叠</el-checkbox>
          <el-checkbox
            v-model="orgNodeAll"
            @change="handleCheckedTreeNodeAll($event, 'org')"
          >全选/全不选</el-checkbox>
          <el-checkbox
            v-model="form.orgCheckStrictly"
            @change="handleCheckedTreeConnect($event, 'org')"
          >父子联动</el-checkbox>
          <el-tree
            ref="org"
            class="tree-border"
...
export default {
  ...
  created() {
    ...
    this.getDicts('sys_auth_scope').then((response) => {
  ...
  methods: {
    ...
    /** 根据角色ID查询组织树结构 */
    getRoleOrgTreeselect(roleId) {
      return roleOrgTreeselect(roleId).then((response) => {
        this.orgOptions = response.orgs
        return response
      })
    },
    // 角色状态修改
    ...
    handleSelectionChange(selection) {
      ...
      this.checked = selection.length === this.roleList.length
    },
    // 树权限（展开/折叠）
    handleCheckedTreeExpand(value, type) {
      if (type == 'menu') {
        ...
        const treeList = this.orgOptions
        for (let i = 0; i < treeList.length; i++) {
    ...
    handleCheckedTreeConnect(value, type) {
      if (type == 'menu') {
        ...
        this.form.orgCheckStrictly = !!value
      }
    },
    /** 新增按钮操作 */
    handleAdd() {
    ...
    handleDataScope(row) {
      this.reset()
      const roleOrgTreeselect = this.getRoleOrgTreeselect(row.roleId)
      getRole(row.roleId).then((response) => {
        this.form = response.data
        this.openDataScope = true
        this.$nextTick(() => {
          roleOrgTreeselect.then((res) => {
    ...
    submitForm: function() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.$confirm(
            `是否确定${this.form.roleId != undefined ? '修改' : '新增'}角色？`,
            '警告',
            {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
