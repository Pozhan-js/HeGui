<template>
  <div class="app-container">
    ...
    <el-table ref="multipleTable" style="margin-top: 20px" v-loading="loading" :data="tableList" :row-key="(row) => row.tableId" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      this.checked = this.tableList.length === selection.length
    },
    /** 修改按钮操作 */
    handleEditTable(row) {
    ...
    handleDelete(row) {
      ...
      this.$confirm(`是否确认${ typeof tableIds === 'object' ? '批量删除' : '删除"' + row.tableComment + '"' }生成?`, "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
